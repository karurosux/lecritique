<script lang="ts">
  ...
  import { AlertCircle, Mail, AlertTriangle, Edit, Calendar, Info } from 'lucide-svelte';
  ...
  $effect(() => {
    if (user?.deactivation_requested_at) {
      ...
      deactivationDate = new Date(requestedAt.getTime() + (15 * 24 * 60 * 60 * 1000)); // Add 15 days
    }
  });
  ...
  async function handleEmailChange(event: Event) {
    ...
    try {
      ...
      if (response.data.data?.token) {
        ...
        onSuccess?.('Email change request sent. Please check your new email for verification.');
      }
  ...
  let isDeactivateEnabled = $derived(deactivateConfirmText.toLowerCase() === 'deactivate');
</script>

<div>
  <div class="mb-8">
    <h2 class="text-2xl font-bold text-gray-900">Account Settings</h2>
    <p class="mt-1 text-sm text-gray-600">Manage your email address and account status</p>
  </div>
  ...
  <div class="space-y-8">
      ...
      <div class="bg-gray-50 rounded-lg p-4">
        <div class="flex items-center justify-between">
          <div class="flex-1">
            ...
            <p class="font-medium text-gray-900">{user?.email || 'No email set'}</p>
    ...
    <div class="border-t pt-8">
      ...
      <div class="bg-red-50 border-2 border-red-200 rounded-lg p-6">
        <h4 class="text-base font-semibold text-red-900 mb-2">Deactivate Account</h4>
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                ...
                <p class="text-sm text-yellow-700">
                  To keep your account active, simply log in before this date or click cancel below.
                </p>
              </div>
            </div>
          </div>
        ...
        {:else}
          <p class="text-sm text-red-700 mb-4">
            Request account deactivation with a 15-day grace period. You can cancel anytime by logging in.
          </p>
        ...
        {:else}
          <div class="space-y-4">
            <div class="bg-white border border-red-300 rounded-lg p-4">
              ...
              <p class="text-sm text-red-700 mb-4">
                This will schedule your account for deactivation after a 15-day grace period.
                You can cancel this request anytime by logging in during the grace period.
              </p>
              
              <div class="space-y-2">
                <label for="deactivate-confirm" class="block text-sm font-medium text-red-900">
                  Type <span class="font-mono bg-red-100 px-1 rounded">deactivate</span> to confirm:
                </label>
                <Input
...
<Modal bind:open={showEmailModal} onClose={closeEmailModal}>
  <div class="w-full max-w-md">
    <div class="mb-6">
      ...
      <p class="mt-1 text-sm text-gray-600">Enter your new email address below</p>
    </div>
    ...
    <form onsubmit={handleEmailChange} class="space-y-4">
      ...
      <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p class="text-sm text-blue-800">
          ...
          A verification link will be sent to both your old and new email addresses.
        </p>
      </div>
      ...
      <div class="flex gap-3 justify-end pt-4">
        ...
        <Button 
          type="submit" 
          variant="gradient"
          disabled={isLoading || !emailForm.newEmail || !emailForm.confirmEmail}
        >
          {#if isLoading}
            <svg class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" fill="none" viewBox="0 0 24 24">
              <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
