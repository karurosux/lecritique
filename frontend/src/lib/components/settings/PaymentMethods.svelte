...
<div>
  <div class="mb-8">
    ...
    <p class="mt-1 text-sm text-gray-600">Manage your payment methods and billing address</p>
  </div>
  ...
  <div class="space-y-4">
    ...
    {#each paymentMethods as method}
      <div class="border-2 rounded-xl p-6 {method.isDefault ? 'border-blue-200 bg-blue-50' : 'border-gray-200'}">
        <div class="flex items-start justify-between">
          <div class="flex items-center space-x-4">
            {#if method.type === 'card'}
              <div class="h-12 w-20 bg-gradient-to-r from-blue-600 to-blue-400 rounded-lg flex items-center justify-center text-white font-bold">
                {method.brand}
              </div>
              <div>
                <p class="font-semibold text-gray-900">•••• •••• •••• {method.last4}</p>
                <p class="text-sm text-gray-600">Expires {method.expiryMonth}/{method.expiryYear}</p>
                {#if method.isDefault}
                  <p class="text-xs text-blue-600 mt-1">Default payment method</p>
                {/if}
              </div>
            {:else}
              <div class="h-12 w-20 bg-gradient-to-r from-gray-600 to-gray-400 rounded-lg flex items-center justify-center text-white">
                <Building class="h-6 w-6" />
              </div>
              <div>
                <p class="font-semibold text-gray-900">Bank Account •••• {method.last4}</p>
                {#if method.isDefault}
                  <p class="text-xs text-blue-600 mt-1">Default payment method</p>
                {/if}
              </div>
    ...
    <div class="mt-8">
      ...
      <div class="grid gap-4 md:grid-cols-2">
        ...
        >
          <CreditCard class="h-8 w-8 text-gray-400 mx-auto mb-2 group-hover:text-gray-500" />
          <p class="text-sm font-medium text-gray-900">Add Credit or Debit Card</p>
        </button>
        ...
        >
          <Building class="h-8 w-8 text-gray-400 mx-auto mb-2 group-hover:text-gray-500" />
