...
<div class="flex items-center gap-2 {className}">
  ...
  <div class="relative">
    ...
    <div class="relative {currentSize.icon} bg-gradient-to-r from-purple-600 via-blue-600 to-blue-700 rounded-lg flex items-center justify-center p-1 shadow-md">
      <!-- Inner background -->
      <div class="absolute inset-1 bg-white rounded"></div>
      
      <!-- QR Code icon -->
      <QrCode size={currentSize.lucide} strokeWidth={2.5} class="text-black relative z-10" />
    </div>
  </div>

  <!-- Logo Text -->
  {#if showText}
    <div class="font-black bg-gradient-to-r from-purple-600 via-blue-600 to-blue-700 bg-clip-text text-transparent {currentSize.text} tracking-tight select-none">
      Kyooar
    </div>
  {/if}
</div>