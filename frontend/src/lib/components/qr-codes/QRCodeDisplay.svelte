<script lang="ts">
  ...
  const qrUrl = browser
    ? `${window.location.origin}/qr/${qrCode.code}`
    : `/qr/${qrCode.code}`;
  ...
  function handlePrint() {
    ...
    printWindow.document.write(
      '<title>QR Codes - ' + qrCode.label + '</title>'
    );
    ...
    printWindow.document.write(
      '.instructions-header h1 { margin: 0 0 10px 0; font-size: 24px; }'
    );
    printWindow.document.write(
    ...
    printWindow.document.write(
      '.qr-item h3 { margin: 10px 0 5px 0; font-size: 14px; }'
    );
    printWindow.document.write(
      '.qr-item .label { font-size: 12px; font-weight: bold; margin: 5px 0; }'
    );
    printWindow.document.write(
      '.qr-item .use-case { font-size: 10px; color: #666; }'
    );
    printWindow.document.write(
    ...
    printWindow.document.write(
      '<h1>' + organizationName + ' - QR Code Sheet</h1>'
    );
    printWindow.document.write(
    ...
    printWindow.document.write(
      '<div class="use-case">Scan to leave feedback</div>'
    );
    printWindow.document.write('</div>');
    printWindow.document.write('</div>');
    printWindow.document.write('</div>');
    ...
    for (let i = 0; i < 2; i++) {
      ...
      printWindow.document.write(
        '<div class="use-case">Share your experience</div>'
      );
      printWindow.document.write('</div>');
    }
    printWindow.document.write('</div>');
    ...
    for (let i = 0; i < 4; i++) {
      ...
      printWindow.document.write(
        '<div class="use-case" style="font-size: 9px;">Feedback: ' +
          qrCode.label +
          '</div>'
      );
      printWindow.document.write('</div>');
  ...
  function handlePrintTentCard() {
    ...
    printWindow.document.write('<html><head>');
    printWindow.document.write(
      '<title>QR Display Stand - ' + qrCode.label + '</title>'
    );
    ...
    printWindow.document.write(
      '.legend h3 { margin-bottom: 10px; color: #333; }'
    );
    printWindow.document.write(
    ...
    printWindow.document.write(
      '.instructions h3 { color: #555; margin: 15px 0 8px 0; }'
    );
    printWindow.document.write(
    ...
    printWindow.document.write(
      '<h1 class="page-title">QR Display Stand Template</h1>'
    );
    ...
    printWindow.document.write(
      '<svg class="template-svg" viewBox="0 0 600 400" xmlns="http://www.w3.org/2000/svg">'
    );
    ...
    // QR code
    printWindow.document.write(
      '<image x="125" y="130" width="130" height="130" href="' +
        qrDataUrl +
        '"/>'
    );
    ...
    printWindow.document.write(
      '<li><strong>Cut:</strong> Cut along all solid black lines</li>'
    );
    printWindow.document.write(
    ...
    <div class="flex gap-2">
      ...
      <div class="relative flex-1">
        ...
        {#if showPrintOptions}
          ...
          <div
            class="absolute top-full mt-2 w-full bg-white border rounded-lg shadow-lg z-20">
            <button
              ...
              <div class="text-gray-500 text-xs">
                Multiple sizes with cut guides
              </div>
            </button>
          </div>
        {/if}
