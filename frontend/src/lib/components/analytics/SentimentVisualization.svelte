<script lang="ts">
  ...
  import { SmileIcon, MehIcon, FrownIcon, TrendingUpIcon, MessageSquareIcon } from 'lucide-svelte';
  ...
  const sentimentData = $derived(() => {
    ...
    const { positive_rate, neutral_rate, negative_rate } = textAnalysis.sentiment_summary;
  ...
  const wordCloud = $derived(() => {
    ...
    textAnalysis.positive_mentions?.forEach(mention => {
      words.push({ text: mention.text, count: mention.count, sentiment: 'positive' });
    });
    
    // Add negative mentions
    textAnalysis.negative_mentions?.forEach(mention => {
      words.push({ text: mention.text, count: mention.count, sentiment: 'negative' });
    });
    
    // Add frequent keywords as neutral
    textAnalysis.frequent_keywords?.forEach(keyword => {
      if (!words.find(w => w.text.toLowerCase() === keyword.keyword.toLowerCase())) {
        words.push({ text: keyword.keyword, count: keyword.count, sentiment: 'neutral' });
      }
    });
...
<Card variant="default" class="sentiment-visualization">
  <div class="mb-6">
    ...
    <p class="text-sm text-gray-600 mt-1">Customer sentiment from text responses</p>
  </div>
  ...
  {:else if sentimentData}
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      {#each sentimentData as sentiment}
        ...
        <div 
          ...
          style="background-color: {hoveredSentiment === sentiment.type ? sentiment.bgColor : '#ffffff'}; border-color: {hoveredSentiment === sentiment.type ? sentiment.color : '#e5e7eb'}"
        ...
        >
          <div class="p-4">
            ...
            <div class="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
              <div 
                ...
                style="width: {sentiment.percentage * animationProgress}%; background-color: {sentiment.color}"
              ></div>
            </div>
          </div>
          ...
          <div 
            ...
            style="background: radial-gradient(circle at {hoveredSentiment === sentiment.type ? '50%' : '150%'} 50%, {sentiment.color} 0%, transparent 70%); transition: all 0.5s ease-out"
          ></div>
        </div>
      {/each}
    </div>
    ...
    {#if sentimentData && sentimentData.length >= 3}
      {@const score = ((sentimentData[0].percentage - sentimentData[2].percentage) + 100) / 2}
      <div class="mt-6 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Based on all text responses</p>
          </div>
