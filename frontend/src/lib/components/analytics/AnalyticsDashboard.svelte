  ...
  const keyMetrics = $derived(() => {
    ...
    const metricsArray = [
      ...
      },
      {
        ...
        format: (v: number) => v != null ? `${v > 0 ? '+' : ''}${v.toFixed(1)}%` : '0.0%',
        change: 0,
        icon: 'improvement'
      }
    ];
      ...
      <div class="p-4">
        ...
        {#if comparison}
          <div class="mt-2 text-xs text-gray-500">
            vs. previous {timeframe === '24h' ? 'day' : timeframe === '7d' ? 'week' : 'month'}
          </div>
        {/if}
      </div>
    </Card>
    ...
    <div class="p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-green-50 rounded-lg p-4">
          <h4 class="text-sm font-medium text-green-900 mb-2">Best Performing Days</h4>
        ...
        <div class="bg-amber-50 rounded-lg p-4">
          <h4 class="text-sm font-medium text-amber-900 mb-2">Attention Needed</h4>
