<script lang="ts">
  ...
  import { CalendarIcon, TrendingUpIcon, TrendingDownIcon } from 'lucide-svelte';
  ...
  function drawTrendChart() {
    ...
    chartData.ratings.forEach((rating, index) => {
      ...
      const progress = Math.min(animationProgress * chartData.ratings.length, index + 1) / (index + 1);
    ...
    // Complete the fill
    ctx.lineTo(padding.left + (chartData.ratings.length - 1) * xStep, canvas.height - padding.bottom);
    ...
    chartData.ratings.forEach((rating, index) => {
      ...
      if (hoveredPoint === index) {
        ...
        const tooltipX = Math.min(x - tooltipWidth / 2, canvas.width - tooltipWidth - 10);
        ...
        ctx.quadraticCurveTo(tooltipX + tooltipWidth, tooltipY, tooltipX + tooltipWidth, tooltipY + 4);
        ctx.lineTo(tooltipX + tooltipWidth, tooltipY + tooltipHeight - 4);
        ctx.quadraticCurveTo(tooltipX + tooltipWidth, tooltipY + tooltipHeight, tooltipX + tooltipWidth - 4, tooltipY + tooltipHeight);
        ctx.lineTo(tooltipX + 4, tooltipY + tooltipHeight);
        ctx.quadraticCurveTo(tooltipX, tooltipY + tooltipHeight, tooltipX, tooltipY + tooltipHeight - 4);
        ...
        ctx.fillText(chartData.labels[index], tooltipX + tooltipWidth / 2, tooltipY + 8);
        
        ctx.font = '11px system-ui, -apple-system, sans-serif';
        ctx.fillText(`Rating: ${rating.toFixed(1)}`, tooltipX + tooltipWidth / 2, tooltipY + 24);
        ctx.fillText(`Reviews: ${chartData.counts[index]}`, tooltipX + tooltipWidth / 2, tooltipY + 36);
      }
    });
  ...
  function handleMouseMove(event: MouseEvent) {
    ...
    if (index >= 0 && index < chartData.labels.length && x >= padding.left && x <= canvas.width - padding.right) {
      hoveredPoint = index;
    } else {
      hoveredPoint = null;
    }
