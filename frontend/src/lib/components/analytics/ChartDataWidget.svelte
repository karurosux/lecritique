<script lang="ts">
  ...
  import { Star, BarChart3, TrendingUp, Utensils, ToggleLeft, ToggleRight, MessageSquare, ThumbsUp, ThumbsDown, Meh } from 'lucide-svelte';
  ...
  <Card variant="elevated">
    <div class="text-center py-20">
      <div class="relative mb-8">
        ...
        <div class="absolute -top-2 -right-2 h-8 w-8 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full flex items-center justify-center">
  ...
  <div class="analytics-charts space-y-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      {#each chartData.charts as chart (chart.question_id)}
        <Card variant="elevated" class="group hover:shadow-lg transition-all duration-200">
          <div class="chart-container">
            ...
            {#if chart.chart_type === 'rating'}
              ...
              <div class="space-y-4">
                {#if chart.data.average}
                  <div class="text-center mb-6 p-5 bg-gradient-to-br from-amber-50 via-orange-50 to-red-50 rounded-2xl border border-amber-200 relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                      <div class="text-3xl font-black text-orange-700 mb-2 tracking-tight">
                        {chart.data.average.toFixed(1)}
                        <span class="text-lg text-orange-600 ml-1">/{chart.data.scale || 5}</span>
                      </div>
                      <div class="text-2xl mb-3 tracking-wider">
                        {getRatingStars(chart.data.average, chart.data.scale)}
                      </div>
                      <div class="inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm px-3 py-1 rounded-full border border-orange-200">
                ...
                {#if chart.data.distribution}
                  <div class="space-y-3">
                    {#each Object.entries(chart.data.distribution).sort(([a], [b]) => parseInt(b) - parseInt(a)) as [rating, count], index}
                      ...
                      <div class="group p-3 hover:bg-orange-50 rounded-xl border border-orange-100 hover:border-orange-200 transition-all duration-200">
                        <div class="flex items-center gap-4">
                          <div class="flex items-center gap-3 w-20 flex-shrink-0">
                            <div class="w-8 h-8 bg-gradient-to-br from-orange-100 to-orange-200 rounded-lg flex items-center justify-center font-bold text-orange-800 text-sm group-hover:scale-105 transition-transform">
                              {rating}
                            </div>
                            <Star class="h-4 w-4 {getColorForRating(parseInt(rating), chart.data.scale)} group-hover:scale-105 transition-transform" />
                          </div>
                          <div class="flex-1 relative">
                            <div class="bg-gradient-to-r from-gray-200 to-gray-100 rounded-xl h-6 overflow-hidden shadow-inner">
                              <div 
                            ...
                            {#if percentage > 20}
                              <div class="absolute inset-0 flex items-center justify-center text-xs font-bold text-white pointer-events-none" style="text-shadow: 0 1px 2px rgba(0,0,0,0.5), 0 0 4px rgba(0,0,0,0.3);">
                                {percentage.toFixed(0)}%
                              </div>
                            {/if}
                          </div>
                          <div class="text-sm font-bold text-gray-900 w-20 text-right bg-white border border-orange-200 px-3 py-2 rounded-lg group-hover:bg-orange-50 transition-colors">
                            {count.toLocaleString()}
                            <span class="block text-xs text-orange-600 font-normal">
            ...
            {:else if chart.chart_type === 'scale'}
              ...
              <div class="space-y-4">
                {#if chart.data.average}
                  <div class="text-center mb-6 p-5 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-2xl border border-blue-200 relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                      <div class="text-3xl font-black text-blue-700 mb-2 tracking-tight">
                        {chart.data.average.toFixed(1)}
                        <span class="text-lg text-blue-600 ml-1">/{chart.data.scale || 5}</span>
                      </div>
                      <div class="inline-flex items-center gap-2 bg-white/80 backdrop-blur-sm px-3 py-1 rounded-full border border-blue-200">
                ...
                {#if chart.data.distribution}
                  <div class="space-y-3">
                    {#each Object.entries(chart.data.distribution).sort(([a], [b]) => parseInt(a) - parseInt(b)) as [value, count], index}
                      ...
                      <div class="group p-3 hover:bg-blue-50 rounded-xl border border-blue-100 hover:border-blue-200 transition-all duration-200">
                        <div class="flex items-center gap-4">
                          <div class="flex items-center gap-3 w-20 flex-shrink-0">
                            <div class="w-8 h-8 bg-gradient-to-br from-blue-100 to-blue-200 rounded-lg flex items-center justify-center font-bold text-blue-800 text-sm group-hover:scale-105 transition-transform">
                              {value}
                            </div>
                            <BarChart3 class="h-4 w-4 text-blue-600 group-hover:scale-105 transition-transform" />
                          </div>
                          <div class="flex-1 relative">
                            <div class="bg-gradient-to-r from-gray-200 to-gray-100 rounded-xl h-6 overflow-hidden shadow-inner">
                              <div 
                          ...
                          <div class="text-sm font-bold text-gray-900 w-20 text-right bg-white border border-blue-200 px-3 py-2 rounded-lg group-hover:bg-blue-50 transition-colors">
                            {count.toLocaleString()}
                            <span class="block text-xs text-blue-600 font-normal">
                              {percentage.toFixed(1)}%
                            </span>
                          </div>
                        </div>
              ...
              {#if chart.data.is_multi_choice && chart.data.combinations && chartViewModes.get(chart.question_id) === 'combinations'}
                ...
                <div class="space-y-3">
                  {#each chart.data.combinations.slice(0, 8) as combo, index}
                    <div class="group p-3 hover:bg-indigo-50 rounded-xl border border-indigo-100 hover:border-indigo-200 transition-all duration-200">
                      <div class="flex items-center gap-4">
                        <div class="w-6 h-6 rounded-lg {getColorForChoice(index)} flex-shrink-0 shadow-sm group-hover:scale-105 transition-transform flex items-center justify-center">
                        ...
                        <div class="flex items-center gap-4">
                          <div class="relative w-28">
                            <div class="bg-gradient-to-r from-gray-200 to-gray-100 rounded-xl h-5 overflow-hidden shadow-inner">
                              <div 
                                class="h-full {getColorForChoice(index)} transition-all duration-700 ease-out rounded-xl relative shadow-sm"
                                style="width: {combo.percentage}%"
                              >
                                <!-- Subtle shine effect -->
                                <div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/30 to-transparent rounded-xl"></div>
                              </div>
                            </div>
                          ...
                          <div class="text-sm font-bold text-gray-900 w-20 text-right bg-white border border-indigo-200 px-3 py-2 rounded-lg group-hover:bg-indigo-50 transition-colors">
                            {combo.count.toLocaleString()}
                            <span class="block text-xs text-indigo-600 font-normal">
                ...
                <div class="space-y-3">
                  {#if chart.data.options}
                    {#each Object.entries(chart.data.options).sort(([,a], [,b]) => b - a) as [option, count], index}
                      ...
                      <div class="group p-3 hover:bg-purple-50 rounded-xl border border-purple-100 hover:border-purple-200 transition-all duration-200">
                        <div class="flex items-center gap-4">
                          <div class="w-6 h-6 rounded-lg {getColorForChoice(index)} flex-shrink-0 shadow-sm group-hover:scale-105 transition-transform flex items-center justify-center">
                            <div class="w-2 h-2 bg-white rounded-full"></div>
                          </div>
                          <span class="text-sm font-semibold text-gray-800 flex-1 truncate">{option}</span>
                          <div class="flex items-center gap-4">
                            <div class="relative w-28">
                              <div class="bg-gradient-to-r from-gray-200 to-gray-100 rounded-xl h-5 overflow-hidden shadow-inner">
                                <div 
                            ...
                            <div class="text-sm font-bold text-gray-900 w-20 text-right bg-white border border-purple-200 px-3 py-2 rounded-lg group-hover:bg-purple-50 transition-colors">
                              {count.toLocaleString()}
                              <span class="block text-xs text-purple-600 font-normal">
                                {percentage.toFixed(1)}%
                              </span>
                            </div>
                          </div>
            ...
            {:else if chart.chart_type === 'text_sentiment'}
              ...
              <div class="space-y-4">
                ...
                <div class="grid grid-cols-3 gap-3 mb-6">
                  <div class="text-center p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border border-green-200 relative overflow-hidden group hover:shadow-lg transition-all duration-200">
                    <div class="absolute top-1 right-1 text-lg opacity-20">😋</div>
                    <div class="w-10 h-10 bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform shadow-lg">
                      <ThumbsUp class="h-5 w-5 text-white" />
                    </div>
                    <div class="text-2xl font-black text-green-700 mb-1">{chart.data.positive || 0}</div>
                    <div class="text-xs font-bold text-green-600 uppercase tracking-wide">Loved It</div>
                  </div>
                  <div class="text-center p-4 bg-gradient-to-br from-amber-50 to-yellow-50 rounded-xl border border-amber-200 relative overflow-hidden group hover:shadow-lg transition-all duration-200">
                    <div class="absolute top-1 right-1 text-lg opacity-20">🤔</div>
                    <div class="w-10 h-10 bg-gradient-to-br from-amber-400 to-yellow-500 rounded-xl flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform shadow-lg">
                      <Meh class="h-5 w-5 text-white" />
                    </div>
                    <div class="text-2xl font-black text-amber-700 mb-1">{chart.data.neutral || 0}</div>
                    <div class="text-xs font-bold text-amber-600 uppercase tracking-wide">It's OK</div>
                  </div>
                  <div class="text-center p-4 bg-gradient-to-br from-red-50 to-rose-50 rounded-xl border border-red-200 relative overflow-hidden group hover:shadow-lg transition-all duration-200">
                    <div class="absolute top-1 right-1 text-lg opacity-20">😞</div>
                    <div class="w-10 h-10 bg-gradient-to-br from-red-500 to-rose-600 rounded-xl flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform shadow-lg">
                      <ThumbsDown class="h-5 w-5 text-white" />
                    </div>
                    <div class="text-2xl font-black text-red-700 mb-1">{chart.data.negative || 0}</div>
                    <div class="text-xs font-bold text-red-600 uppercase tracking-wide">Not Great</div>
                  </div>
                </div>
