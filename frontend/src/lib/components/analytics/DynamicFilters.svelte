    ...
    {#if hasActiveFilters()}
      <div class="mt-4 p-3 bg-blue-50 rounded-lg">
        <div class="text-sm text-blue-900">
          ...
          <span class="ml-2">
            {currentFilters.timeframe !== '7d'
              ? `${timeframeOptions.find(o => o.value === currentFilters.timeframe)?.label}`
              : ''}
            {currentFilters.segments.timeOfDay !== 'all'
              ? `, ${segmentOptions.timeOfDay.find(o => o.value === currentFilters.segments.timeOfDay)?.label}`
              : ''}
            {currentFilters.segments.dayType !== 'all'
              ? `, ${segmentOptions.dayType.find(o => o.value === currentFilters.segments.dayType)?.label}`
              : ''}
            {currentSelection.comparisonMode !== 'none'
              ? `, ${comparisonModes.find(m => m.value === currentSelection.comparisonMode)?.label}`
              : ''}
          </span>
        </div>
      </div>
