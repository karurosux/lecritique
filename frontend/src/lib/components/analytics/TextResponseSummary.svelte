<script lang="ts">
  ...
  import { MessageSquareIcon, ThumbsUpIcon, ThumbsDownIcon, HashIcon } from 'lucide-svelte';
...
<Card variant="default" class="text-response-summary">
  <div class="mb-6">
    ...
    <p class="text-sm text-gray-600 mt-1">Key themes, sentiment analysis, and recent feedback</p>
  </div>
          ...
          <div class="flex items-center">
            ...
            <div>
              ...
              <div class="text-lg font-semibold text-green-600">
                {(textAnalysis.sentiment_summary.positive_rate || 0).toFixed(1)}%
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="flex items-center">
            ...
            <div>
              ...
              <div class="text-lg font-semibold text-red-600">
                {(textAnalysis.sentiment_summary.negative_rate || 0).toFixed(1)}%
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="space-y-2">
            {#each topPositive as mention}
              <div class="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg px-3 py-2">
                <span class="text-sm text-green-800 truncate">{mention.text}</span>
                <span class="text-xs text-green-600 ml-2">({mention.count})</span>
              </div>
            {/each}
          </div>
        {:else}
          <div class="text-gray-500 text-sm">No positive mentions available</div>
        {/if}
      </div>
    </div>
                ...
                {#if feedback.rating}
                  <div class="ml-3 flex items-center gap-1">
                    <div class="flex items-center gap-1">
                      {#each Array(5) as _, i}
                        <div class="w-3 h-3 {i < feedback.rating ? 'text-yellow-400' : 'text-gray-300'}">
                          ⭐
                        </div>
                      {/each}
                    </div>
