<script lang="ts">
    ...
    data.series.forEach((series: any) => {
      ...
      if (questionType === 'general') {
        if (series.metric_type.includes('rating_questions'))
          questionType = 'rating';
        else if (series.metric_type.includes('scale_questions'))
          questionType = 'scale';
        else if (series.metric_type.includes('yes_no_questions'))
          questionType = 'yes_no';
        else if (series.metric_type.includes('text_questions'))
          questionType = 'text';
        else if (series.metric_type.includes('single_choice_questions'))
          questionType = 'choice';
        else if (series.metric_type.includes('multiple_choice_questions'))
          questionType = 'choice';
      ...
      console.log(
        `Series ${series.metric_type} assigned to group: ${questionType}`
      );
  ...
  function getGroupTitle(questionType: string): string {
    switch (questionType) {
      case 'rating':
        return 'Rating Questions (1-5 Stars)';
      case 'scale':
      ...
      case 'choice':
        return 'Choice Questions (Response Counts)';
      case 'general':
  ...
  function getGroupDescription(questionType: string): string {
    switch (questionType) {
      case 'rating':
        return 'Average star ratings over time (1-5 scale)';
      case 'scale':
        return 'Average scale responses over time (1-10 scale)';
      case 'yes_no':
        return 'Percentage of "Yes" responses over time';
      case 'text':
        return 'Sentiment scores from text analysis (-1 to +1)';
      case 'choice':
        return 'Number of responses for choice-based questions';
      case 'general':
        return 'Overall survey response counts and general metrics';
      default:
        return `Metrics for ${questionType} type questions`;
    }
  }
  ...
  $effect(() => {
    ...
    console.log(
      'SmartTimeSeriesChart - groupedSeries keys:',
