        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              ...
              label: (context: any) =>
                `${context.dataset.label}: ${formatCountValue(context.parsed.y)}`,
            },
          },
        },
        scales: {
            ...
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-semibold"
                  >{formatCountValue(seriesData.statistics.average)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Total:</span>
                <span class="font-semibold"
                  >{formatCountValue(seriesData.statistics.total)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Peak:</span>
                <span class="font-semibold"
                  >{formatCountValue(seriesData.statistics.max)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Minimum:</span>
                <span class="font-semibold"
                  >{formatCountValue(seriesData.statistics.min)}</span>
              </div>
