<script lang="ts">
  ...
  import { BarChart3, TrendingUp, TrendingDown, Minus } from 'lucide-svelte';
  ...
  function createChart() {
    ...
    const datasets = data.series.map((seriesData: any, index: number) => {
      ...
      return {
        ...
        data: points,
        borderColor: `hsl(${260 + index * 50}, 70%, 50%)`, // Purple tones for scale
        backgroundColor: `hsl(${260 + index * 50}, 70%, 50%)20`,
        borderWidth: 3,
        pointRadius: 6,
        pointHoverRadius: 8,
        fill: false,
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              ...
              label: (context: any) => `${context.dataset.label}: ${formatScaleValue(context.parsed.y)}`
            }
          }
        },
        scales: {
          ...
          y: {
            ...
            title: { display: true, text: 'Average Scale Score' }
          }
        }
      }
    };
...
<div class="scale-chart bg-white rounded-lg border p-6">
  <div class="flex items-center gap-2 mb-4">
    <BarChart3 class="w-5 h-5 text-purple-500" />
    <h3 class="text-lg font-semibold text-gray-900">Scale Analysis (1-10)</h3>
  </div>
          ...
          <div class="bg-purple-50 border border-purple-200 rounded-lg p-4">
            ...
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-semibold">{formatScaleValue(seriesData.statistics.average)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Highest:</span>
                <span class="font-semibold">{formatScaleValue(seriesData.statistics.max)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Lowest:</span>
                <span class="font-semibold">{formatScaleValue(seriesData.statistics.min)}</span>
              </div>
