  ...
  function createChart() {
    ...
    const datasets = data.series.map((seriesData: any, index: number) => {
      ...
      return {
        ...
        data: points,
        borderColor: `hsl(${45 + index * 60}, 70%, 50%)`,
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              ...
              label: (context: any) =>
                `${context.dataset.label}: ${formatRatingValue(context.parsed.y)}`,
            },
          },
        },
        scales: {
            ...
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-semibold"
                  >{formatRatingValue(seriesData.statistics.average)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Highest:</span>
                <span class="font-semibold"
                  >{formatRatingValue(seriesData.statistics.max)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Lowest:</span>
                <span class="font-semibold"
                  >{formatRatingValue(seriesData.statistics.min)}</span>
              </div>
