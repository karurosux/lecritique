        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              ...
              label: (context: any) => `${context.dataset.label}: ${formatYesNoValue(context.parsed.y)}`
            }
          }
        },
        scales: {
...
<div class="yesno-chart bg-white rounded-lg border p-6">
  <div class="flex items-center gap-2 mb-4">
    ...
    <h3 class="text-lg font-semibold text-gray-900">Yes/No Response Analysis</h3>
  </div>
          ...
          <div class="bg-green-50 border border-green-200 rounded-lg p-4">
            ...
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                ...
                <span class="font-semibold">{formatYesNoValue(seriesData.statistics.average)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Highest Yes:</span>
                <span class="font-semibold">{formatYesNoValue(seriesData.statistics.max)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Lowest Yes:</span>
                <span class="font-semibold">{formatYesNoValue(seriesData.statistics.min)}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-600">Avg. No Rate:</span>
                <span class="font-semibold">{(100 - seriesData.statistics.average).toFixed(1)}%</span>
              </div>
