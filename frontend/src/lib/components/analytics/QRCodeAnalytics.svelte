<script lang="ts">
  ...
  import { QrCode, MessageSquare, Star, TrendingUp, MapPin, Clock, Users, AlertCircle, MessageCircle, ThumbsUp } from 'lucide-svelte';
  ...
  const qrMetrics = $derived(() => {
    ...
    const totalScans = analyticsData.total_qr_scans || qrData.reduce((sum, qr) => sum + qr.scans_count, 0);
    const totalFeedback = analyticsData.total_feedback || qrData.reduce((sum, qr) => sum + qr.feedback_count, 0);
    const activeCount = analyticsData.total_active_codes || qrData.filter(qr => qr.is_active).length;
    const avgConversion = analyticsData.average_conversion_rate || 
    ...
    const qrDataEnhanced = qrData.map(qr => {
      ...
      const ratings = qrFeedbacks.map(f => f.overall_rating || f.rating || 0).filter(r => r > 0);
      const avgRating = ratings.length > 0 ? ratings.reduce((sum, r) => sum + r, 0) / ratings.length : 0;
      const withComments = qrFeedbacks.filter(f => f.comment && f.comment.trim().length > 0).length;
      const responseQuality = qrFeedbacks.length > 0 ? (withComments / qrFeedbacks.length) * 100 : 0;
    ...
    const sortedByResponses = [...qrDataEnhanced].sort((a, b) => b.feedback_count - a.feedback_count);
    const topResponseGenerators = sortedByResponses.slice(0, 5);
    
    // Find QR codes with quality issues
    const qualityIssues = qrDataEnhanced.filter(qr => 
  ...
  <Card variant="elevated" padding={false}>
    <div class="p-6">
      <div class="mb-6">
        ...
        <p class="text-sm text-gray-600 mt-1">How each QR code generates quality feedback responses</p>
      </div>
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  <div class="flex items-start gap-3">
                    <div class="h-10 w-10 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-lg flex items-center justify-center shadow-sm">
                      <QrCode class="h-5 w-5 text-white" />
                    </div>
                    <div class="flex-1">
                      <h4 class="font-medium text-gray-900">{qr.label || 'Unnamed QR Code'}</h4>
                      {#if qr.location}
                        <div class="flex items-center gap-1 mt-1">
                          <MapPin class="h-3 w-3 text-gray-400" />
                          <span class="text-sm text-gray-600">{qr.location}</span>
                        </div>
                      {/if}
                      ...
                      <div class="flex items-center gap-4 mt-3">
                        <div class="flex items-center gap-1">
                          ...
                          <span class="text-sm font-medium text-gray-700">{qr.feedback_count} responses</span>
                        </div>
                        
                        {#if hasRating}
                          <div class="flex items-center gap-1">
                            <Star class="h-4 w-4 {ratingColor} fill-current" />
                            <span class="text-sm font-medium {ratingColor}">{qr.average_rating.toFixed(1)}</span>
                          </div>
                        {/if}
                        ...
                        {#if qr.response_quality != null && qr.feedback_count > 0}
                          <div class="flex items-center gap-1">
                            ...
                            <span class="text-sm text-gray-600">{qr.response_quality.toFixed(0)}% with comments</span>
                          </div>
                        {/if}
                      </div>
                      ...
                      <div class="flex items-center gap-3 mt-2 text-xs text-gray-500">
                        ...
                        {#if !qr.is_active}
                          <span class="px-2 py-0.5 bg-gray-100 text-gray-600 font-medium rounded">Inactive</span>
                        {/if}
                      </div>
                    </div>
                  </div>
                ...
                <div class="text-right">
                  <div class="space-y-2">
                    <div>
                      <p class="text-xs text-gray-500 uppercase tracking-wide">Response Rate</p>
                      <p class="text-2xl font-bold {getConversionColor(qr.conversion_rate)}">
                ...
                {#if qr.response_quality < 30}
                  <div class="mt-3 pt-3 border-t border-amber-200">
                    <div class="flex items-center gap-2 text-sm text-amber-700">
                      ...
                      <span>Low comment rate - Consider encouraging more detailed feedback</span>
                    </div>
                  </div>
                {/if}
              ...
              {:else if qr.feedback_count === 0 && qr.scans_count > 5}
                <div class="mt-3 pt-3 border-t border-gray-200">
                  <div class="flex items-center gap-2 text-sm text-gray-600">
                    <AlertCircle class="h-4 w-4" />
                    <span>No responses yet despite {qr.scans_count} scans - Review QR code messaging</span>
                  </div>
                </div>
              {/if}
            </div>
        ...
        {#if qrMetrics().qualityIssues.length > 0}
          <div class="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
            <div class="flex items-start gap-3">
              ...
              <div>
                <h4 class="font-medium text-amber-900">Response Quality Insights</h4>
                <p class="text-sm text-amber-700 mt-1">
                  {qrMetrics().qualityIssues.length} QR code{qrMetrics().qualityIssues.length > 1 ? 's' : ''} 
                ...
                <ul class="text-sm text-amber-700 mt-2 space-y-1 list-disc list-inside">
                  ...
                  <li>Low comment rates suggest customers need more encouragement to share details</li>
                  <li>Consider adding incentives for detailed feedback</li>
                  <li>Review the feedback form to ensure it's easy to complete</li>
                </ul>
              </div>
            </div>
          </div>
        ...
        {#if qrMetrics().topResponseGenerators.length > 0}
          <div class="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
            <div class="flex items-start gap-3">
              ...
              <div>
                <h4 class="font-medium text-green-900">Top Response Generators</h4>
                ...
                <ul class="text-sm text-green-700 mt-2 space-y-1">
                  {#each qrMetrics().topResponseGenerators.slice(0, 3) as qr}
                    <li>
                      ...
                      {#if qr.location} at {qr.location}{/if}
                      - {qr.feedback_count} responses
                      {#if qr.average_rating > 0} ({qr.average_rating.toFixed(1)} ⭐){/if}
                    </li>
                  {/each}
                </ul>
              </div>
      ...
      {:else}
        <div class="text-center py-8">
          ...
          <p class="text-sm text-gray-400 mt-2">Create and deploy QR codes to start tracking performance.</p>
        </div>
      {/if}
    </div>
  </Card>
