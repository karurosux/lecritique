  ...
  <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-6">
      ...
      <div class="flex items-center space-x-3">
        ...
        <div>
          ...
          <div class="flex items-center space-x-4 mt-1">
            <p class="text-gray-600 font-medium">Manage your dishes and menu items</p>
            {#if !loading}
              <div class="flex items-center space-x-3 text-sm">
                <div class="flex items-center space-x-1">
                  <div class="w-2 h-2 bg-green-400 rounded-full"></div>
                  <span class="text-gray-600">{dishes.filter(d => d.is_available).length} Available</span>
                </div>
                <div class="flex items-center space-x-1">
                  <div class="w-2 h-2 bg-gray-400 rounded-full"></div>
                  <span class="text-gray-600">{dishes.filter(d => !d.is_available).length} Hidden</span>
                </div>
              </div>
            {/if}
          </div>
    ...
    <div class="flex items-center space-x-3">
      ...
      >
        ...
        <span class="relative z-10">Add Dish</span>
      </Button>
    </div>
  </div>
</div>