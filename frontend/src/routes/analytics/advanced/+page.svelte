<script lang="ts">
  ...
  let comparisonFilters = $state({
    period1Start: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split('T')[0],
    period1End: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split('T')[0],
    period2Start: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
      .toISOString()
      .split('T')[0],
  ...
  async function loadTimeSeriesData() {
    ...
    try {
      ...
      const params = {
        ...
        ...(timeSeriesFilters.productId && {
          product_id: timeSeriesFilters.productId,
        }),
      };
  ...
  async function collectMetrics() {
    ...
    try {
      const api = getApiClient();
      await api.api.v1AnalyticsOrganizationsCollectMetricsCreate(
        selectedOrganization
      );
...
<div class="advanced-analytics max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
  <!-- Header -->
  <div
    class="mb-8 flex flex-col lg:flex-row lg:justify-between lg:items-center gap-6">
    <div class="space-y-3">
      <div class="flex items-center space-x-3">
        ...
        <div>
          ...
          <p class="text-gray-600 font-medium">
            Time series analysis and comparisons of survey responses
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="flex gap-3">
      ...
      <Button
        ...
        <RefreshCw
          class={`w-4 h-4 mr-2 ${collectingMetrics ? 'animate-spin' : ''}`} />
  ...
  <Card variant="elevated" class="mb-8 overflow-hidden">
    <div
      class="bg-gradient-to-br from-purple-50 via-pink-50 to-indigo-50 p-6 rounded-2xl m-1">
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-sm text-gray-600">
              Select the organization to analyze
            </p>
          </div>
        </div>
      ...
      {#if selectedOrganization && organizations.find(org => org.id === selectedOrganization)}
        <div
          class="mt-4 pt-4 border-t border-purple-100/50 flex items-center gap-6">
          <span class="flex items-center gap-2 text-sm">
            ...
            <span class="text-gray-600"
              >Products: <span class="font-semibold text-gray-900"
                >{availableProducts.length}</span
              ></span>
          </span>
          <span class="flex items-center gap-2 text-sm">
            <div
              class="h-8 w-8 bg-pink-100 rounded-lg flex items-center justify-center">
              <BarChart3 class="w-4 h-4 text-pink-600" />
            </div>
            <span class="text-gray-600"
              >Questions: <span class="font-semibold text-gray-900"
                >{availableQuestions.length}</span
              ></span>
          </span>
        </div>
      {/if}
    ...
    <Card variant="elevated" class="mb-6">
      <div class="p-6 border-b border-gray-100">
        ...
        <p class="text-sm text-gray-600 mt-1">
          Configure date range, granularity, and select questions to analyze
        </p>
      </div>
          ...
          <div class="border-t border-gray-100 pt-6">
            <div class="flex items-center justify-between mb-4">
              ...
              <div class="text-xs text-gray-500">
                {timeSeriesFilters.metricTypes.filter(t =>
                  t.startsWith('question_')
                ).length} questions selected
              </div>
            </div>
            ...
            <div class="space-y-3">
              {#each Object.entries(availableProductGroups) as [productName, productData]}
                <Card variant="minimal" padding={false} class="overflow-hidden">
                  <details class="group">
                    <summary
                      class="cursor-pointer flex items-center justify-between p-4 hover:bg-gray-50 transition-colors duration-200 list-none [&::-webkit-details-marker]:hidden">
                      <div class="flex items-center gap-3">
                        <div
                          class="h-6 w-6 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-md flex items-center justify-center">
                        ...
                        <div>
                          ...
                          <p class="text-xs text-gray-500">
                            {productData.individualQuestions.length} question{productData
                              .individualQuestions.length !== 1
                              ? 's'
                              : ''}
                          </p>
                        </div>
                      ...
                      <div class="flex items-center gap-2">
                        <span
                          ...
                          {productData.individualQuestions.filter(q =>
                            timeSeriesFilters.metricTypes.includes(q.value)
                          ).length} selected
                        </span>
                        <ChevronDown
                          class="w-4 h-4 text-gray-400 group-open:rotate-180 transition-transform duration-200" />
                      </div>
                    </summary>
                    ...
                    <div class="border-t border-gray-100 bg-gray-50/50 p-4">
                      <div class="space-y-2 max-h-48 overflow-y-auto">
                        {#each productData.individualQuestions as question}
                          <label
                            class="flex items-center gap-2 cursor-pointer hover:bg-white p-2 rounded-md transition-colors">
                            <input
                              type="checkbox"
                              class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500 focus:ring-offset-0"
                              checked={timeSeriesFilters.metricTypes.includes(
                                question.value
                              )}
                              onchange={e => {
                                if (e.target.checked) {
                                  ...
                                  timeSeriesFilters.metricTypes =
                                    timeSeriesFilters.metricTypes.filter(
                                      t => t !== question.value
                                    );
                                }
                              }} />
                            <div class="flex-1 min-w-0">
                              <span class="text-sm text-gray-900 block truncate"
                                >{question.label}</span>
                            </div>
                          </label>
                        {/each}
        ...
        {#if Object.keys(availableProductGroups).length > 0}
          <div class="border-t border-gray-100 pt-6">
            <div class="flex items-center justify-between mb-4">
              ...
              <div class="text-xs text-gray-500">
                {comparisonFilters.metricTypes.filter(t =>
                  t.startsWith('question_')
                ).length} questions selected
              </div>
            </div>
                ...
                <Card variant="minimal" padding={false} class="overflow-hidden">
                  <details class="group">
                    <summary
                      ...
                      <div class="flex items-center gap-3">
                        <div
                          class="h-6 w-6 bg-gradient-to-br from-purple-500 to-pink-600 rounded-md flex items-center justify-center">
                          <Activity class="w-3 h-3 text-white" />
                        </div>
                        <div>
                          <h5 class="font-medium text-gray-900">
                      ...
                      <div class="flex items-center gap-2">
                        <span
                          ...
                          {productData.individualQuestions.filter(q =>
                            comparisonFilters.metricTypes.includes(q.value)
                          ).length} selected
                        </span>
                        <ChevronDown
                          class="w-4 h-4 text-gray-400 group-open:rotate-180 transition-transform duration-200" />
                        ...
                        {#each productData.individualQuestions as question}
                          <label
                            ...
                            <input
                              ...
                              checked={comparisonFilters.metricTypes.includes(
                                question.value
                              )}
                              onchange={e => {
                                if (e.target.checked) {
                                  ...
                                  comparisonFilters.metricTypes =
                                    comparisonFilters.metricTypes.filter(
                                      t => t !== question.value
                                    );
                                }
                              }} />
