<script lang="ts">
  ...
  import { Loader2, MapPin, Building2, CheckCircle, UtensilsCrossed, ChevronRight, Lightbulb, Menu, MessageSquare, ArrowLeft, X } from 'lucide-svelte';
  ...
  interface Question {
    id: string;
    text: string;
    type: 'rating' | 'scale' | 'single_choice' | 'multiple_choice' | 'yes_no' | 'text';
    is_required: boolean;
    ...
    {:else if error}
      ...
      <Card>
        <div class="text-center py-8 sm:py-12 px-4">
          <svg class="h-10 w-10 sm:h-12 sm:w-12 text-red-500 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
          </svg>
          <h2 class="text-lg sm:text-xl font-semibold text-gray-900 mb-2">Invalid QR Code</h2>
      ...
      <div class="space-y-2">
        ...
        <div class="py-2 sm:py-3 px-2">
          <div class="flex items-start gap-3 bg-white/60 backdrop-blur-sm rounded-2xl p-3 border border-white/30">
            ...
            {:else}
              <div class="h-10 w-10 sm:h-12 sm:w-12 rounded-2xl bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center shadow-sm flex-shrink-0 ring-2 ring-white/50">
                ...
                <div class="text-xs text-gray-500">
                  <div class="flex items-center">
                    ...
                    {#if qrData.location.address}
                      <span class="ml-1 text-gray-400 hidden sm:inline">• {qrData.location.address}</span>
                    {/if}
                  </div>
                  {#if qrData.location.address}
        ...
        <div class="p-2 sm:p-3">
          <div class="text-center mb-3 sm:mb-4">
            <div class="inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-blue-500 via-purple-500 to-purple-600 text-white mb-3 sm:mb-4 shadow-lg shadow-blue-500/25 ring-2 ring-white/20">
              ...
              {#each dishesWithQuestions as dish}
                ...
                >
                  ...
                  <div class="relative flex items-start justify-between">
                    <div class="flex-1 min-w-0">
                      <div class="flex items-start gap-4">
                        <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-br from-purple-400 to-pink-400 flex items-center justify-center flex-shrink-0 shadow-lg group-hover:shadow-xl transition-shadow">
                          <UtensilsCrossed class="h-6 w-6 sm:h-7 sm:w-7 text-white" />
                        </div>
                    ...
                    <div class="ml-4 flex-shrink-0 opacity-50 group-hover:opacity-100 transition-opacity">
        ...
        <div class="bg-white rounded-3xl p-5 shadow-sm mb-6">
          <div class="flex items-center gap-4">
            {#if qrData.restaurant?.logo}
              ...
              />
            {:else}
              <div class="h-14 w-14 rounded-2xl bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center shadow-md">
          ...
          <form onsubmit={handleSubmit} class="space-y-4">
            ...
            {#each [...questions].sort((a, b) => a.display_order - b.display_order) as question, index}
              <div class="bg-white rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300 relative overflow-hidden group question-{question.type}">
                <div class="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-purple-500 to-pink-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                <div class="space-y-4">
                  <div class="flex items-start gap-3">
                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-purple-100 to-pink-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <span class="text-sm font-bold text-purple-700">{index + 1}</span>
                    </div>
                    <label class="block text-lg font-semibold text-gray-900 flex-1">
                      {question.text}
                      {#if question.is_required}
                  ...
                  {:else if question.type === 'scale'}
                    <div class="px-3">
                      ...
                      <div class="flex justify-between text-sm mt-3">
                        <span class="text-gray-500 font-medium">{question.min_label || question.min_value || 1}</span>
                        <span class="font-bold text-purple-600 text-base bg-purple-50 px-3 py-1 rounded-full">{responses[question.id] || 0}</span>
                        <span class="text-gray-500 font-medium">{question.max_label || question.max_value || 10}</span>
                      </div>
                    </div>
                  ...
                  {:else if question.type === 'single_choice' && question.options}
                    <div class="space-y-3 px-3">
                      {#each question.options as option}
                        <label class="flex items-center cursor-pointer p-4 rounded-2xl border-2 bg-white hover:bg-gray-50 transition-all duration-200 
                  ...
                  {:else if question.type === 'multi_choice' && question.options}
                    <div class="space-y-3 px-3">
                      {#each question.options as option}
                        ...
                        <label class="flex items-center cursor-pointer p-4 rounded-2xl border-2 transition-all duration-200
                                     {isSelected 
                                       ? 'border-purple-500 bg-purple-50' 
                  ...
                  {:else if question.type === 'yes_no'}
                    <div class="flex gap-4 px-3">
                      ...
                      <button
                        ...
                        class="flex-1 py-4 px-6 rounded-2xl font-medium transition-all duration-200 {responses[question.id] === true ? 'bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-lg shadow-green-500/25' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}"
                      >
                      ...
                      <button
                        ...
                        class="flex-1 py-4 px-6 rounded-2xl font-medium transition-all duration-200 {responses[question.id] === false ? 'bg-gradient-to-r from-red-500 to-pink-500 text-white shadow-lg shadow-red-500/25' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}"
                      >
                        <div class="flex items-center justify-center gap-2">
                  ...
                  {:else if question.type === 'text'}
                    <div class="px-3">
                      ...
                      <div class="flex justify-between items-center mt-2 text-xs text-gray-500">
                        ...
                        <span class="{(responses[question.id]?.length || 0) > 450 ? 'text-orange-500' : ''} {(responses[question.id]?.length || 0) > 480 ? 'text-red-500' : ''}">
            ...
            <!-- Additional Comments -->
            <div class="bg-white rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
              <div class="flex items-center gap-3 mb-4">
                ...
                <label for="comment" class="block text-lg font-semibold text-gray-900">
                  Additional Comments
                </label>
              </div>
              <textarea
              ...
              <div class="flex justify-between items-center mt-2 text-xs text-gray-500">
                <span>Maximum 500 characters</span>
                <span class="{(comment?.length || 0) > 450 ? 'text-orange-500' : ''} {(comment?.length || 0) > 480 ? 'text-red-500' : ''}">
            ...
            <div class="bg-white rounded-3xl p-6 shadow-sm hover:shadow-md transition-shadow duration-300">
              <div class="flex items-center gap-3 mb-2">
                <div class="w-8 h-8 rounded-full bg-gradient-to-br from-green-100 to-emerald-100 flex items-center justify-center">
                  <Menu class="h-4 w-4 text-green-600" />
                </div>
                <label for="email" class="block text-lg font-semibold text-gray-900">
                  Email (Optional)
                </label>
              </div>
              <p class="text-sm text-gray-600 mb-4 ml-11">
                Leave your email to receive exclusive coupons and rewards for your feedback
              </p>
              <input
                id="email"
                type="email"
            ...
            <!-- Error Display -->
            {#if error}
              <div class="bg-gradient-to-r from-red-50 to-pink-50 border-2 border-red-200 rounded-2xl p-5">
