<script lang="ts">
  ...
  async function loadRestaurant() {
    ...
    try {
      ...
      // Get restaurant details via API
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const response = await api.api.v1RestaurantsUpdate(restaurantId, {
        name: formData.name,
      ...
      if (response.data.success) {
        // Redirect to restaurants list on success
        goto('/restaurants');
      } else {
...
<svelte:head>
  <title>Edit Restaurant - LeCritique</title>
  <meta name="description" content="Edit restaurant information" />
</svelte:head>
    ...
    {:else if error && !formData.name}
      ...
      <Card>
        <div class="text-center py-12">
          ...
          <h3 class="text-lg font-medium text-gray-900 mb-2">Failed to load restaurant</h3>
          <p class="text-gray-600 mb-4">{error}</p>
          <div class="flex justify-center space-x-3">
            <Button on:click={loadRestaurant}>Try Again</Button>
