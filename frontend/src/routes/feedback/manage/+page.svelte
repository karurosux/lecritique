<script lang="ts">
  ...
  function toggleCollapse(feedbackId: string) {
    ...
    collapsedStates[feedbackId] =
      collapsedStates[feedbackId] === undefined
        ? false
        : !collapsedStates[feedbackId];
  }
    ...
    try {
      ...
      if (selectedOrganization) {
        ...
        const productPromises = organizations.map(async organization => {
          try {
            const response = await api.api.v1OrganizationsProductsList(
              organization.id
            );
            return response.data.data || [];
          } catch (err) {
            console.error(
              `Error loading products for organization ${organization.id}:`,
              err
            );
            return [];
          }
      ...
      if (selectedOrganization) {
        ...
        const feedbackPromises = organizations.map(async organization => {
          try {
            ...
            console.error(
              `Error loading feedback for organization ${organization.id}:`,
              err
            );
            return [];
          }
    ...
    <div class="space-y-6">
      {#each feedback as fb, index}
        <div
          ...
          <div
            class="absolute -inset-0.5 bg-gradient-to-r from-blue-500 to-purple-600 rounded-2xl opacity-0 group-hover:opacity-20 blur transition duration-500">
          </div>
          <Card
            variant="default"
            class="relative overflow-hidden border-0 shadow-xl hover:shadow-2xl transition-all duration-500">
            <!-- Header Section -->
            <div class="relative">
              <!-- Background gradient accent -->
              <div
                class="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-blue-500/5 to-purple-600/5 rounded-full blur-3xl">
              </div>

              <div
                class="relative flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
                <!-- Left side info -->
                      ...
                      <div
                        class="absolute inset-0 bg-gradient-to-r {fb.rating >= 4
                          ...
                          : fb.rating >= 3
                            ...
                            : 'from-red-400 to-pink-500'} rounded-xl blur opacity-25">
                      </div>
                      <div
                        class="relative flex items-center gap-2 px-4 py-2 bg-gradient-to-r {fb.rating >=
                        4
                          ? 'from-green-50 to-emerald-50 border-green-200'
                          : fb.rating >= 3
                            ? 'from-yellow-50 to-orange-50 border-yellow-200'
                            : 'from-red-50 to-pink-50 border-red-200'} border rounded-xl">
              ...
              {#if fb.comment}
                <div class="relative mb-6">
                  <div
                    class="absolute -left-1 top-0 bottom-0 w-1 bg-gradient-to-b from-blue-500 to-purple-600 rounded-full">
                  </div>
                  <div class="pl-6">
                ...
                <div class="space-y-4">
                  ...
                  <button
                    ...
                    <div class="flex items-center gap-3">
                      ...
                      <div class="text-left">
                        <h4
                          class="text-lg font-semibold text-gray-900 group-hover/header:text-purple-700 transition-colors duration-200">
                          Customer Responses
                        </h4>
                        <p
                          class="text-sm text-gray-500 group-hover/header:text-gray-600 transition-colors duration-200">
                          {fb.responses.length} question{fb.responses.length > 1
                            ? 's'
                            : ''} answered
                        </p>
                      </div>
                  ...
                  {#if isCollapsed(fb.id)}
                    <div class="ml-8 text-sm text-gray-400 italic">
                      Click above to view {fb.responses.length} detailed response{fb
                        .responses.length > 1
                        ? 's'
                        : ''}
                    </div>
                      ...
                      {#each fb.responses as response, index}
                        <div class="group/question relative">
                          ...
                          <div class="absolute -left-3 top-0 z-10">
                            <div
                              class="w-6 h-6 bg-gradient-to-r from-purple-500 to-blue-600 rounded-full flex items-center justify-center">
                          ...
                          <!-- Question Card -->
                          <div
                            class="ml-6 bg-gradient-to-r from-gray-50 to-white border border-gray-200 rounded-xl p-5 group-hover/question:shadow-md transition-all duration-300">
                            <!-- Question Text -->
                            <div class="mb-3">
                              <p
                                class="text-sm font-medium text-gray-900 leading-relaxed">
                            ...
                            <div class="space-y-2">
                              <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                  {#if typeof response.answer === 'boolean'}
                                    <div
                                      class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg {response.answer
                                  ...
                                  {:else if typeof response.answer === 'number'}
                                    ...
                                    {:else}
                                      <!-- Numeric value -->
                                      <div
                                        class="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-100 text-blue-800 border border-blue-200 rounded-lg">
                                        <span class="font-semibold"
                                  ...
                                  {:else if Array.isArray(response.answer)}
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                      {#each response.answer as item}
                                        <span
                                          class="inline-flex items-center px-3 py-1.5 bg-purple-100 text-purple-800 border border-purple-200 rounded-lg text-sm font-medium">
                                          {item}
                                        </span>
                                      {/each}
                                    </div>
